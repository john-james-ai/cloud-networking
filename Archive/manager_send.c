...
int main(int argc, char** argv)
{
	...
	short no_destID = htons(destID);

	short senderSocket = socket(AF_INET, SOCK_DGRAM, 0);
	if(senderSocket < 0)
		perror("socket()");
	...
	if(!strcmp(argv[2], "cost"))
	{
		...
		short no_newCost = htonl(atoi(argv[4]));
	...
	{
		short msgLen = 4+sizeof(short int)+strlen(argv[4]);
