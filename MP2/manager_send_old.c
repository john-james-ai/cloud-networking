...
int main(int argc, char** argv)
{
	...
	short destID = atoi(argv[3]);
	int no_destID = htonl(destID);
	...
	if(!strcmp(argv[2], "cost"))
	{
		...
		char sendBuf[4+sizeof(int)+sizeof(int)];

		strcpy(sendBuf, "cost");
		memcpy(sendBuf+4, &no_destID, sizeof(int));
		memcpy(sendBuf+4+sizeof(int), &no_newCost, sizeof(int));

		if(sendto(senderSocket, sendBuf, 4+sizeof(int)+sizeof(int), 0,
		          (struct sockaddr*)&destAddr, sizeof(destAddr)) < 0)
			perror("sendto()");
	}
	else
	{
		int msgLen = 4+sizeof(int)+strlen(argv[4]);
		char* sendBuf = (char *)malloc(msgLen);

		strcpy(sendBuf, "send");
		memcpy(sendBuf+4, &no_destID, sizeof(int));
		memcpy(sendBuf+4+sizeof(int), argv[4], strlen(argv[4]));
